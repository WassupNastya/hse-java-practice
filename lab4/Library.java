...
public class Library implements Serializable {
    ...
    public boolean edit(String id, BookTypes type, Scanner in) {
        ...
        if (type == BookTypes.TITLE) {
            library.get(id).setTitle(data);
            System.out.println("Success!");
        }
        if (type == BookTypes.AUTHOR) {
            library.get(id).setAuthor(data);
            System.out.println("Success!");
        }
        if (type == BookTypes.LANGUAGE) {
            library.get(id).setLanguage(data);
            System.out.println("Success!");
        }
        try {
